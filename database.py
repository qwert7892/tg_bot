...
class Db:
    ...
    def add_user(self, tg_username, telegram_id, full_name, chat):
        with self.connection:
            return self.cursor.execute(
                "INSERT INTO `bases` (`tg_username`, `telegram_id`,`full_name`, 'chat_id') VALUES(?,?,?,?)",
                (tg_username, telegram_id, full_name, chat))

    def create_profile(self, telegram_id, tg_username, name, description, age, sign, city, photo, sex):
        with self.connection:
            return self.cursor.execute("INSERT INTO `profile_list` (`telegram_id`,`telegram_username`,`name`,"
                                       "`description`, 'age', 'sign', `city`,`photo`,`sex`) VALUES(?,?,?,?,?,?,?,?,?)",
                                       (telegram_id, tg_username, name, description, age, sign, city, photo, sex))
    ...
    def add_like_exists(self, send, recieve):
        with self.connection:
            result = self.cursor.execute('SELECT * FROM `like` WHERE `send` = ? AND `recieve` = ?',
                                         (send, recieve)).fetchall()
            return bool(len(result))

    def add_like(self, send, recieve):
        with self.connection:
            return self.cursor.execute('INSERT INTO `like` (`send`,`recieve`) VALUES(?,?)', (send, recieve))
